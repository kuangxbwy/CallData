...
public abstract class BaseDao {
    ...
    protected int genRegionNum( String tel, String date ) {
        ...
        // 13301234567
        String usercode = tel.substring(tel.length()-4);//电话号码的后四位
        // 20181010120000
        String yearMonth = date.substring(0, 6);//比如20年3月打的电话数据
        //取二者的hash值
        ...
        // 取模
        int regionNum = crc % ValueConstant.REGION_COUNT;//ValueConstant.REGION_COUNT为分区数量。不适合用位运算，因为分区数必须为2的幂次方才行。
    ...
    private byte[][] genSplitKeys(int regionCount) {
        ...
        //Collections.sort(bsList, new Bytes.ByteArrayComparator()); 对bsList排序，默认是排序的
